...
export async function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const loggedInNavigation: NavigationItem[] = [
    ...
    ...navigation.filter((item) => item.href !== "/" && item.href !== "/dashboard")
  ];
    ...
    <>
      <header className="sticky top-0 z-40 border-b border-white/10 bg-[#050816]/78 backdrop-blur-2xl">
        <nav className="container flex min-h-20 items-center justify-between gap-4">
          <Link href={user ? "/dashboard" : "/"} className="flex items-center gap-3 font-extrabold tracking-wide">
          ...
          <div className="flex items-center gap-2">
            {user ? (
              ...
              <>
                <Link href="/login" className="hidden rounded-full px-4 py-2 text-sm font-bold text-white/70 transition hover:text-white sm:inline-flex">
                  Login
                </Link>
                <Link href="/signup" className="rounded-full blue-purple px-5 py-3 text-sm font-extrabold shadow-[0_14px_34px_rgba(91,140,255,.28)]">
                  Start Free
                </Link>
              </>
            )}
