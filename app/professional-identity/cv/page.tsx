...
export default async function ProfessionalCvPage() {
  ...
  const [unlocked, canExport, identityReadModel, discoveryResult] = await Promise.all([
  ...
  const defaultTemplateName = "PATHZY Signature Professional";
  ...
  const initialCvDocument = professionalIdentityCvDocument(identityReadModel.values, {
    templateName: defaultTemplateName,
    lastUpdated
  });
  const cvSyncStatus = professionalIdentityCvSyncStatus(identityReadModel.values, lastUpdated);
  ...
  return (
    <div className="page-pad mx-auto w-full max-w-[1560px] px-4 sm:px-6 lg:px-8">
      <ProfessionalIdentityTool
        ...
        defaultOptions={{ cvType: "Entry-Level CV", language: "english", templateName: defaultTemplateName }}
        fields={[
