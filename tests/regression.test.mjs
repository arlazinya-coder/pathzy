...
assert.match(professionalIdentityTool, /tool === "cover-letter" \? "grid gap-5 lg:grid-cols-2 lg:items-start"/, "Cover Letter workspace must stay single-column on mobile and use top-aligned side-by-side columns on desktop.");
...
assert.match(professionalIdentityTool, /tool === "cover-letter" \? "lg:self-start"/, "Cover Letter editor must keep its natural height and align to the top of the workspace.");
assert.match(professionalIdentityTool, /<Card className="lg:sticky lg:top-24 lg:self-start lg:h-fit">/, "Cover Letter preview must be top-aligned, sticky on desktop, and use intrinsic content height.");
const coverLetterWorkspaceImplementation = professionalIdentityTool.slice(professionalIdentityTool.indexOf('{tool === "cover-letter" ? ('), professionalIdentityTool.indexOf('{tool === "cover-letter" ? renderCoverLetterTemplateGallery()'));
assert.doesNotMatch(coverLetterWorkspaceImplementation, /(?:xl|lg):h-\[calc\(100vh-136px\)\]|(?:xl|lg):min-h-0|(?:xl|lg):overflow-y-auto|self-stretch|items-stretch/, "Cover Letter Studio must not couple preview height to editor height or add nested editor/preview scrolling.");
assert.match(professionalIdentityTool, /mx-auto mt-5 max-w-\[920px\] overflow-hidden rounded-\[22px\] bg-\[#dfe7f3\] p-2 text-black sm:p-3/, "Cover Letter A4 preview must stay centered and scale inside mobile width without horizontal scrolling.");
