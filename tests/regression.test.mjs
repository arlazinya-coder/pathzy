...
assert.match(professionalIdentityTool, /tool === "cover-letter" \? "xl:col-span-1 xl:h-\[calc\(100vh-136px\)\] xl:min-h-0 xl:overflow-y-auto"/, "Cover Letter editor must use a fixed viewport-controlled desktop height and scroll internally only on desktop.");
assert.match(professionalIdentityTool, /<Card className="xl:sticky xl:top-24 xl:col-span-3 xl:h-\[calc\(100vh-136px\)\] xl:min-h-0 xl:self-start xl:overflow-y-auto">/, "Cover Letter preview must be sticky and independently scrollable in the 75% desktop column.");
