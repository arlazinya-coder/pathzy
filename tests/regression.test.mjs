...
const referencesClient = readFileSync("components/professional-identity/references-client.tsx", "utf8");
const supportingDocumentsClient = readFileSync("components/professional-identity/supporting-documents-client.tsx", "utf8");
const followUpEmailPage = readFileSync("app/professional-identity/follow-up-email/page.tsx", "utf8");
const referencesPage = readFileSync("app/professional-identity/references/page.tsx", "utf8");
const supportingDocumentsPage = readFileSync("app/professional-identity/supporting-documents/page.tsx", "utf8");
const readinessEngine = readFileSync("lib/pathzy-brain/readiness-engine.ts", "utf8");
...
assert.match(roadmapPage, /const nextActionPrimaryLabel = nextAction\?\.destinationRoute === appRoutes\.professionalIdentityCv \? "Build My CV" : "Continue My Journey";/, "My Employment Journey must show Build My CV when the shared next action is the CV Builder.");
assert.match(roadmapPage, /<ButtonLink href=\{nextAction\.destinationRoute\}>\{nextActionPrimaryLabel\}<\/ButtonLink>/, "My Employment Journey must send its primary CTA to the shared next action route.");
...
assert.match(professionalIdentityPage, /<ProfileActionEditor rows=\{profileRows\} \/>/, "Professional Profile review cards must use the inline profile action editor.");
assert.match(profileActionEditor, /fetch\("\/api\/professional-profile"/, "Profile edit actions must save inside the Professional Profile workflow.");
assert.match(profileActionEditor, /Save returns here with your updated information/, "Saving profile actions must return users to My Professional Profile.");
assert.match(profileActionEditor, /Cancel keeps your current information unchanged/, "Canceling profile actions must not send users away.");
assert.match(professionalProfileApi, /upsert\(profileUpdate, \{ onConflict: "user_id" \}\)/, "Profile edits must safely create or update the user's profile row.");
assert.match(professionalProfileApi, /from\("discovery_responses"\)[\s\S]*answers/, "Profile edits for skills, projects, experience, achievements, and references must save to the user's journey answers.");
assert.doesNotMatch(`${professionalIdentityPage}\n${profileActionEditor}`, /appRoutes\.settings|appRoutes\.billing|\/dashboard|\/profile"/, "Profile edit actions must not redirect to Settings, Billing, Dashboard, or legacy Profile.");
...
assert.match(professionalIdentityTool, /Build My Cover Letter/, "Completed CVs must offer Build My Cover Letter as a next action.");
assert.match(professionalIdentityTool, /Improve My LinkedIn/, "Completed CVs and cover letters must offer Improve My LinkedIn as a next action.");
assert.match(professionalIdentityTool, /Back to My Employment Journey/, "Document builders must let users return to My Employment Journey.");
assert.match(professionalIdentityTool, /Your cover letter is ready\. Keep moving\./, "Completed cover letters must show clear next actions.");
assert.match(professionalIdentityTool, /function LinkedInPreview/, "LinkedIn must use a LinkedIn-specific preview instead of the CV renderer.");
assert.match(professionalIdentityTool, /LinkedIn Headline|LinkedIn Preview/, "LinkedIn preview must contain LinkedIn-specific sections.");
assert.match(professionalIdentityTool, /Copy Headline/, "LinkedIn page must support copying the headline.");
assert.match(professionalIdentityTool, /Copy About Section/, "LinkedIn page must support copying the About section.");
assert.match(professionalIdentityTool, /Copy Full LinkedIn Content/, "LinkedIn page must support copying the full LinkedIn content.");
assert.match(professionalIdentityTool, /AI Improve/, "LinkedIn page must include an AI Improve action.");
assert.match(readinessEngine, /const onboardingScore = onboardingCompleted \? 10 : 0;/, "Employment readiness must award 10 points for onboarding completion.");
assert.match(readinessEngine, /const professionalProfileScore = profileComplete \? 25 : 0;/, "Employment readiness must award 25 points for completing My Professional Profile.");
assert.match(readinessEngine, /const cvScore = cvComplete \? 25 : 0;/, "Employment readiness must award 25 points for CV completion.");
assert.match(readinessEngine, /const coverLetterScore = coverLetterComplete \? 15 : 0;/, "Employment readiness must award 15 points for cover letter completion.");
assert.match(readinessEngine, /const linkedinScore = linkedinComplete \? 10 : 0;/, "Employment readiness must award 10 points for LinkedIn completion.");
assert.match(readinessEngine, /const opportunityScore = opportunityStarted \? 10 : 0;/, "Employment readiness must award 10 points for the first saved or applied opportunity.");
assert.match(readinessEngine, /const interviewScore = interviewComplete \? 5 : 0;/, "Employment readiness must award 5 points for interview preparation.");
assert.match(readinessEngine, /cv_readiness_score: cvScore \+ coverLetterScore/, "Cover letter completion must add to document readiness instead of reducing the score.");
...
assert.match(professionalIdentityTool, /renderCoverLetterHtmlFromData\(previewCoverLetterData\)/, "Cover Letter preview must use coverLetterData.");
...
assert.match(routes, /professionalIdentityFollowUpEmail: "\/professional-identity\/follow-up-email"/, "Follow-up Email must have the requested dedicated route.");
assert.match(routes, /professionalIdentityReferences: PATHZY_ROUTES\.REFERENCES/, "References must have a dedicated Professional Identity route.");
assert.match(routes, /professionalIdentitySupportingDocuments: PATHZY_ROUTES\.SUPPORTING_DOCUMENTS/, "Supporting Documents must have a dedicated Professional Identity route.");
assert.match(followUpEmailPage, /requireAuthenticatedUser\("\/professional-identity\/follow-up-email"\)/, "Follow-up Email must load at the requested route.");
assert.match(referencesPage, /<ReferencesClient/, "References route must use its own references workflow.");
assert.match(supportingDocumentsPage, /<SupportingDocumentsClient/, "Supporting Documents route must use its own upload workflow.");
assert.match(professionalIdentityTool, /function PremiumDocumentPreview/, "Non-CV professional documents must use dedicated previews.");
assert.match(professionalIdentityTool, /tool === "recruiter-message"[\s\S]*Recruiter Message/, "Recruiter Message must have a recruiter-message-specific preview.");
assert.match(professionalIdentityTool, /tool === "follow-up"[\s\S]*Follow-up Email/, "Follow-up Email must have a follow-up-specific preview.");
assert.match(professionalIdentityTool, /tool === "career-passport"[\s\S]*Career Passport/, "Career Passport must have a career-passport-specific preview.");
assert.doesNotMatch(professionalIdentityTool, /renderCvHtml\(document\.content/, "Professional Identity Tool must not reuse the CV preview renderer for other document types.");
assert.match(professionalIdentityTool, /if \(exportLocked\) \{[\s\S]*setExportUpgradeRequired\(true\);[\s\S]*return;[\s\S]*\}[\s\S]*await navigator\.clipboard\.writeText/, "Full-content copy must be gated for free users.");
assert.match(professionalIdentityTool, /Copy Full LinkedIn Profile/, "LinkedIn must expose a full-profile copy action.");
assert.match(professionalIdentityTool, /Copy Recruiter Message/, "Recruiter Message must expose a recruiter-message copy action.");
assert.match(professionalIdentityTool, /Copy Follow-up Email/, "Follow-up Email must expose a follow-up email copy action.");
assert.match(referencesClient, /Add Reference/, "References workflow must allow adding references.");
assert.match(referencesClient, /Delete Reference/, "References workflow must allow deleting references.");
assert.match(referencesClient, /AI Formatting/, "References workflow must include AI formatting.");
assert.match(referencesClient, /Download PDF/, "References workflow must include Premium PDF download.");
assert.match(supportingDocumentsClient, /Unsupported file type/, "Supporting Documents must validate file type.");
assert.match(supportingDocumentsClient, /10MB/, "Supporting Documents must validate file size.");
assert.match(supportingDocumentsClient, /Replace/, "Supporting Documents must allow replacing documents.");
assert.match(supportingDocumentsClient, /Rename document/, "Supporting Documents must allow renaming documents.");
assert.match(supportingDocumentsClient, /Delete/, "Supporting Documents must allow deleting documents.");
